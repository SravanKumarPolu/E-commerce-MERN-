        ...
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{formatCurrency(data.summary.averageOrderValue)}</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {data.paymentBreakdown.map((payment) => (
              <div key={payment._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900">{payment._id}</p>
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm text-gray-600">
                    {((payment.total / data.summary.totalSales) * 100).toFixed(1)}%
                  </p>
                </div>
              </div>
            ))}
          ...
          {data.statusBreakdown.map((status) => (
            <div key={status._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-gray-600">
                  {((status.count / data.summary.totalOrders) * 100).toFixed(1)}%
                </p>
              </div>
            </div>
          ))}
        ...
        <div className="space-y-3">
          {data.salesData.map((day) => (
            <div key={day._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                <p className="font-semibold text-gray-900">{formatCurrency(day.totalSales)}</p>
                <p className="text-sm text-gray-600">Avg: {formatCurrency(day.averageOrderValue)}</p>
              </div>
            </div>
          ))}
        </div>
