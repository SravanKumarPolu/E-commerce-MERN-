    ...
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center p-3 sm:p-4 md:p-6 transition-all duration-1000 ease-out ${
      ...
      <div className={`w-full max-w-sm sm:max-w-md lg:max-w-lg transition-all duration-700 ease-out delay-200 ${
        ...
        <div className={`text-center mb-4 sm:mb-6 lg:mb-8 transition-all duration-500 ease-out delay-300 ${
          ...
          <div className={`inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-gradient-to-r from-blue-700 to-indigo-700 rounded-xl sm:rounded-2xl mb-3 sm:mb-4 lg:mb-6 shadow-lg sm:shadow-xl transition-all duration-300 hover:scale-110 hover:shadow-2xl hover:rotate-3 ${
        ...
        <div className={`bg-white rounded-xl sm:rounded-2xl shadow-lg sm:shadow-xl p-4 sm:p-5 lg:p-6 xl:p-8 transition-all duration-500 ease-out delay-400 ${
          ...
          <form onSubmit={onSubmitHandler} className="space-y-3 sm:space-y-4 lg:space-y-5">
            {/* Error Message */}
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-900 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-sm font-medium animate-shake relative overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-red-100/50 to-red-200/50 animate-pulse-slow"></div>
              </div>
            )}

            {/* Success Message */}
            {success && (
              <div className="bg-green-50 border border-green-200 text-green-900 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-sm font-medium animate-bounce relative overflow-hidden">
                <div className="flex items-center space-x-2 relative z-10">
                ...
                {/* Subtle background animation */}
                <div className="absolute inset-0 bg-gradient-to-r from-green-100/50 to-green-200/50 animate-pulse-slow"></div>
              </div>
            )}

            {/* Loading Skeleton - Enhanced */}
            {isFormSubmitting && !isLoading && (
              <div className="space-y-3 sm:space-y-4 lg:space-y-5 animate-pulse">
            ...
            {currentState === "Sign Up" && !isFormSubmitting && (
              <div className={`space-y-1.5 sm:space-y-2 transition-all duration-500 ease-out ${
                ...
                <label className="block text-sm sm:text-base font-semibold text-gray-900 transition-colors duration-200 hover:text-blue-700">
                  Full Name
                </label>
                <div className="relative group">
                  <input
                    type="text"
                    className={`w-full pl-10 sm:pl-12 pr-4 py-2.5 sm:py-3 lg:py-4 text-sm sm:text-base lg:text-lg border rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all duration-300 bg-white text-gray-900 placeholder-gray-600 group-hover:border-blue-400 group-hover:shadow-md ${
                      fieldErrors.name 
                        ? 'border-red-400 focus:ring-red-500 animate-shake' 
                        : name.length > 0 && !fieldErrors.name 
                          ? 'border-green-400 focus:ring-green-500'
                          : 'border-gray-400 focus:ring-blue-600'
                    } ${focusedField === 'name' ? 'ring-2 ring-blue-600 ring-opacity-50 shadow-lg' : ''}`}
                ...
                {fieldErrors.name && (
                  <div className="text-red-700 text-xs sm:text-sm font-medium mt-1 animate-fadeIn flex items-center space-x-1">
                    <svg className="w-3 h-3 text-red-600 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    ...
                    className={`w-full pl-10 sm:pl-12 pr-4 py-2.5 sm:py-3 lg:py-4 text-sm sm:text-base lg:text-lg border rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all duration-300 bg-white text-gray-900 placeholder-gray-600 group-hover:border-blue-400 group-hover:shadow-md ${
                      fieldErrors.email 
                        ...
                        : email.length > 0 && !fieldErrors.email 
                          ? 'border-green-400 focus:ring-green-500'
                          : 'border-gray-400 focus:ring-blue-600'
                    } ${focusedField === 'email' ? 'ring-2 ring-blue-600 ring-opacity-50 shadow-lg' : ''}`}
              ...
              <div className="space-y-1.5 sm:space-y-2">
                ...
                <div className="relative group">
                  <input
                    ...
                    className={`w-full pl-10 sm:pl-12 pr-10 sm:pr-12 py-2.5 sm:py-3 lg:py-4 text-sm sm:text-base lg:text-lg border rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all duration-300 bg-white text-gray-900 placeholder-gray-600 group-hover:border-blue-400 group-hover:shadow-md ${
                      fieldErrors.password 
                        ? 'border-red-400 focus:ring-red-500 animate-shake' 
                        : password.length > 0 && !fieldErrors.password 
                          ? 'border-green-400 focus:ring-green-500'
                          : 'border-gray-400 focus:ring-blue-600'
                    } ${focusedField === 'password' ? 'ring-2 ring-blue-600 ring-opacity-50 shadow-lg' : ''}`}
                  ...
                  <button
                    type="button"
                    className="absolute inset-y-0 right-0 pr-3 flex items-center hover:bg-gray-50 rounded-r-lg sm:rounded-r-xl transition-all duration-200 hover:scale-110 active:scale-95 hover:shadow-inner"
                ...
                {currentState === "Sign Up" && password && !isFormSubmitting && (
                  <div className={`mt-2 sm:mt-3 p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl border border-gray-200 transition-all duration-500 ease-out animate-slideDown hover:shadow-md`}>
            ...
            {!isFormSubmitting && (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0 text-xs sm:text-sm lg:text-base">
                <button
                  type="button"
                  className="text-blue-700 hover:text-blue-800 font-semibold transition-all duration-200 hover:underline text-left hover:scale-105 active:scale-95 hover:bg-blue-50 px-2 py-1 rounded-lg"
                >
                  Forgot password?
                </button>
                <button
                  ...
                  className="text-blue-700 hover:text-blue-800 font-semibold transition-all duration-200 hover:underline text-left sm:text-right hover:scale-105 active:scale-95 hover:bg-blue-50 px-2 py-1 rounded-lg"
                >
            ...
            <div className="pt-2 sm:pt-3 lg:pt-4">
              <button 
                type="submit" 
                disabled={isLoading}
                className={`w-full py-3 sm:py-4 lg:py-5 px-4 sm:px-6 text-white text-sm sm:text-base lg:text-lg font-bold rounded-lg sm:rounded-xl shadow-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 transform ${
                  isLoading
                    ? 'bg-blue-600 cursor-not-allowed opacity-75'
                    : 'bg-blue-700 hover:bg-blue-800 hover:shadow-xl hover:scale-105 active:scale-95 focus:ring-blue-600'
                }`}
